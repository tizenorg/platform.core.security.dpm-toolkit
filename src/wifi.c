...
int wifi_set_profile_change_state_handler(struct xtk_policy* self)
{
	...
	if (xtk_open_radio_popup(self, GET_STATE_CHANGE_OPTIONS, &allow) == BUTTON_CANCEL) {
	...
	if (state == allow) {
		dpm_context_release_wifi_policy(context, policy);
		dpm_context_destroy(context);
		xtk_open_message_popup(self, GET_STATE_CHANGE_MESSAGE(state));
...
int wifi_set_network_access_restriction_handler(struct xtk_policy* self)
{
	...
	if (xtk_open_radio_popup(self, GET_RESTRICTION_MODE_OPTIONS, &enable) == BUTTON_CANCEL) {
		return POLICY_RESULT_FAIL;
	}
	...
	if (state == enable) {
		dpm_context_release_wifi_policy(context, policy);
		dpm_context_destroy(context);
		xtk_open_message_popup(self, GET_RESTRICTION_MODE_MESSAGE(state));
		return POLICY_RESULT_NONE;
	}
...
int wifi_add_ssid_to_blocklist_handler(struct xtk_policy* self)
{
	...
	if (xtk_open_entry_popup(self, "SSID", &entry) == BUTTON_CANCEL) {
	...
	if (dpm_wifi_add_ssid_to_blocklist(policy, entry) == 0) {
		dpm_context_release_wifi_policy(context, policy);
		dpm_context_destroy(context);
		open_message_popup(policy_get_title(self), entry);
		return POLICY_RESULT_NONE;
	}
