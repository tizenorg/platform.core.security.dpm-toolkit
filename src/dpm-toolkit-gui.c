...
Evas_Object* create_basic_popup(Evas_Object* parent, const char* title, void* data)
{
...
Evas_Object* create_entry_box(Evas_Object* parent, const char* msg)
{
...
Evas_Object* create_radio_group(Evas_Object* parent, char* text[], int nr)
{
	int i;
	...
	for (i = 0; i < nr; i++) {
		...
		//evas_object_smart_callback_add(radio, "changed", on_radio_button_clicked, (void *)i);
	}
...
void create_button_group(Evas_Object* parent, int mask, void* data)
{
	Evas_Object* button;
...
int open_radio_popup(const char* title, char* text[], int nr, int* index)
{
	int done = 0;
	Evas_Object* popup = NULL;
...
void open_message_popup(const char* title, const char* message)
{
	int done = 0;
	Evas_Object* popup = NULL;
	...
	create_button_group(naviframe, BUTTON_OK, (void *)done);
	evas_object_show(popup);
...
static void _gl_policy_group_select(void* data, Evas_Object* obj, void* event_info)
{
	...
	foreach_policy(policy_list, selected_group) {
		xtk_policy_t* entity = glist_to_policy(policy_list);
		if (is_invalid_policy(entity)) {
			continue;
		}
...
Evas_Object* create_genlist(Evas_Object* naviframe)
{
	Evas_Object* widget;
	Elm_Genlist_Item_Class* itc;
...
void win_terminate_request_cb(void* data, Evas_Object* obj, void* event_info)
{
	ui_app_exit();
}
...
static int create_base_gui(appdata_s* ad)
{
	...
	eext_object_event_callback_add(naviframe, EEXT_CALLBACK_BACK, eext_naviframe_back_cb, ad);
	...
	elm_naviframe_item_pop_cb_set(nfitem, naviframe_pop_cb, ad);
