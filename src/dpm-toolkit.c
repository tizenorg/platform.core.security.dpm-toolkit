...
char* state_change_radio_options[2] = {
	"Allow State Change",
	"Disallow State Change"
};

char* restriction_mode_radio_options[2] = {
...
int xtk_open_radio_popup(xtk_policy_t* policy, char* radio_text[], int nr, int* index)
{
	...
	ret = open_radio_popup(title, radio_text, nr, index);

	if (radio_text == state_change_radio_options ||
		radio_text == restriction_mode_radio_options)
		*index = *index ? 0 : 1;
...
char* state_change_radio_results[2] = {
	"Allowed",
	"Disallowed"
};

char* restriction_mode_radio_results[2] = {
	"Enabled",
	"Disabled"
};
...
{
	open_message_popup(policy_get_title(policy), popup_message);
}
