...
int create_zone_handler(struct dpm_toolkit_entity* self)
{
        ...
        name = self->entry_input;
        wizappid = (char*)xmlGetProp(self->model, (xmlChar*)"wizappid");

        if (name == NULL || wizappid == NULL) {
                dlog_print(DLOG_ERROR, LOG_TAG, "can't find parameter");
                return POLICY_RESULT_FAIL;
        }

        ret = dpm_create_zone(handle, name, wizappid);
...
int destroy_zone_handler(struct dpm_toolkit_entity* self)
{
        ...
        ret = dpm_remove_zone(handle, name);
