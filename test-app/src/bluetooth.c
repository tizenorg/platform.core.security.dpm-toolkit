...
int add_device_to_blacklist_handler(struct dpm_toolkit_entity* self)
{
	...
	mac_address = (char*)xmlGetProp(self->model, (xmlChar*)"default");
	if (mac_address == NULL) {
...
int unset_device_restriction_handler(struct dpm_toolkit_entity* self)
{
	dlog_print(DLOG_DEBUG, LOG_TAG, "unset_device_restriction_handler");
	...
	if (dpm_bluetooth_set_device_restriction(policy, false) < 0) {
		dlog_print(DLOG_DEBUG, LOG_TAG, "Failed to unset device restriction");
		dpm_context_release_bluetooth_policy(policy);
		dpm_context_destroy(context);
		return POLICY_RESULT_FAIL;
	}
...
int is_device_restricted_handler(struct dpm_toolkit_entity* self)
{
	...
	int allowed = dpm_bluetooth_is_device_restricted(policy);
	if (allowed < 0) {
	...
	//TODO(seok85.hong) : display allow state
	dlog_print(DLOG_DEBUG, LOG_TAG, "TODO: popup allow state. (allow: %d)", allowed);
...
int add_uuid_to_blacklist_handler(struct dpm_toolkit_entity* self)
{
	...
	char* uuid = NULL;
	uuid = (char*)xmlGetProp(self->model, (xmlChar*)"default");
	if (uuid == NULL) {
...
int unset_uuid_restriction_handler(struct dpm_toolkit_entity* self)
{
	dlog_print(DLOG_DEBUG, LOG_TAG, "unset_uuid_restriction_handler");
	...
	if (dpm_bluetooth_set_uuid_restriction(policy, false) < 0) {
		dlog_print(DLOG_DEBUG, LOG_TAG, "Failed to unset uuid restriction");
		dpm_context_release_bluetooth_policy(policy);
		dpm_context_destroy(context);
		return POLICY_RESULT_FAIL;
	}
...
int is_uuid_restricted_handler(struct dpm_toolkit_entity* self)
{
	...
	int allowed = dpm_bluetooth_is_uuid_restricted(policy);
	if (allowed < 0) {
...
dpm_toolkit_entity_t dpm_toolkit_bluetooth_policy[] = {
	...
	{
		.id ="UNSET_DEVICE_RESTRICTION",
		.handler = unset_device_restriction_handler
	},
	{
	...
	{
		.id ="UNSET_UUID_RESTRICTION",
		.handler = unset_uuid_restriction_handler
	},
	{
