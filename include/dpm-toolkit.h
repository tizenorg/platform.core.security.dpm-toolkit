...
({														\
	...
	strcat(buffer, state_change_results[!status]);		\
})

#define RESTRICTION_MODE_MESSAGE(status)			 	\
({														\
	char buffer[64] = "Restriction Mode ";				\
	strcat(buffer, restriction_mode_results[!status]);	\
})
...
extern const char* state_change_results[2];
extern const char* restriction_mode_results[2];
